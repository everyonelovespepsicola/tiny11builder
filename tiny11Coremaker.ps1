...
if ($input -eq 'y') {
    ...
    if ($mountedImages) {
        foreach ($img in $mountedImages) {
            if ($img.Path -eq "$mainOSDrive\scratchdir") {
                Write-Host "Cleaning up previously mounted image in scratchdir..."
                [GC]::Collect()
                ...
                Dismount-WindowsImage -Path "$mainOSDrive\scratchdir" -Discard -ErrorAction SilentlyContinue | Out-Null
            }
        }
    }
    & 'dism.exe' '/English' '/Cleanup-Wim' | Out-Null
    if (Test-Path "$mainOSDrive\scratchdir") { Remove-Item -Path "$mainOSDrive\scratchdir" -Recurse -Force -ErrorAction SilentlyContinue }
    if (Test-Path "$mainOSDrive\tiny11") { Remove-Item -Path "$mainOSDrive\tiny11" -Recurse -Force -ErrorAction SilentlyContinue }
    ...
    Get-ChildItem -Path "$DriveLetter\" -Force | Copy-Item -Destination "$mainOSDrive\tiny11" -Recurse -Force | Out-Null
    ...
    $packagePrefixes = 'Clipchamp.Clipchamp_', 'Microsoft.BingNews_', 'Microsoft.BingWeather_', 'Microsoft.GamingApp_', 'Microsoft.GetHelp_', 'Microsoft.Getstarted_', 'Microsoft.MicrosoftOfficeHub_', 'Microsoft.MicrosoftSolitaireCollection_', 'Microsoft.People_', 'Microsoft.PowerAutomateDesktop_', 'Microsoft.Todos_', 'Microsoft.WindowsAlarms_', 'microsoft.windowscommunicationsapps_', 'Microsoft.WindowsFeedbackHub_', 'Microsoft.WindowsMaps_', 'Microsoft.WindowsSoundRecorder_', 'Microsoft.Xbox.TCUI_', 'Microsoft.XboxGamingOverlay_', 'Microsoft.XboxGameOverlay_', 'Microsoft.XboxSpeechToTextOverlay_', 'Microsoft.YourPhone_', 'Microsoft.ZuneMusic_', 'Microsoft.ZuneVideo_', 'MicrosoftCorporationII.MicrosoftFamily_', 'MicrosoftCorporationII.QuickAssist_', 'MicrosoftTeams_', 'Microsoft.549981C3F5F10_', 'Microsoft.Windows.Copilot', 'MSTeams_', 'Microsoft.OutlookForWindows_', 'Microsoft.Windows.Teams_', 'Microsoft.Copilot_', 'Microsoft.Windows.Recall_'
    ...
    Write-Host "Removing MS Paint (System32):"
    if (Test-Path "$mainOSDrive\scratchdir\Windows\System32\mspaint.exe") {
        & 'takeown' '/f' "$mainOSDrive\scratchdir\Windows\System32\mspaint.exe" >null
        & 'icacls' "$mainOSDrive\scratchdir\Windows\System32\mspaint.exe" '/grant' "$($adminGroup.Value):(F)" '/T' '/C' >null
        Remove-Item -Path "$mainOSDrive\scratchdir\Windows\System32\mspaint.exe" -Force -ErrorAction SilentlyContinue >null
    }
    ...
    & 'reg' 'add' 'HKLM\zSOFTWARE\Policies\Microsoft\Edge' '/v' 'EdgeCopilotEnabled' '/t' 'REG_DWORD' '/d' '0' '/f' | Out-Null
    ...
    if (Test-Path $nuclearScript) {
        & $nuclearScript -MountPath "$mainOSDrive\scratchdir"
    }
    ...
    Write-Host "Injecting custom WinPE installer..."
    $winpeshlPath = "$mainOSDrive\scratchdir\Windows\System32\winpeshl.ini"
    ...
    $installerPath = "$mainOSDrive\scratchdir\Windows\System32\installer.cmd"
    $installerContent = @"
@echo off
color 1f
echo ===================================================
...
"@
    ...
    Write-Host "Copying unattended file for bypassing OOBE..."
    Copy-Item -Path "$PSScriptRoot\autounattend.xml" -Destination "$mainOSDrive\tiny11\autounattend.xml" -Force | Out-Null
    Write-Host "Creating ei.cfg to bypass edition prompt..."
    "[EditionID]`r`n`r`n[Channel]`r`nRetail`r`n[VL]`r`n0" | Out-File -FilePath "$mainOSDrive\tiny11\sources\ei.cfg" -Encoding ASCII
    ...
    Remove-Item -Path "$mainOSDrive\tiny11" -Recurse -Force -ErrorAction SilentlyContinue | Out-Null
    Remove-Item -Path "$mainOSDrive\scratchdir" -Recurse -Force -ErrorAction SilentlyContinue | Out-Null
