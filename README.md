...
- **🖥️ Interactive GUI (`tiny11GUI.ps1`)**: A visual wrapper that lets you easily select which components to keep or remove (Edge, OneDrive, Telemetry, Copilot, etc.) without editing any code. You can also configure local accounts, passwords, and choose your Windows Edition directly from the UI.
- **☢️ Nuclear Option (`DisableNuclear.ps1`)**: An aggressive optional toggle that completely disables Windows Updates, Windows Recall AI, and blocks the Update Medic Service from repairing disabled updates.
- **📦 Custom Software & Updates Injection**: Drop `.msu` update files into the `updates` folder to slipstream them into the image. Place `.exe`, `.msi`, or `.ps1` installers into the `software` folder to have them automatically installed post-setup.
- **🧹 Advanced Cleanup Tool (`cleanup_scratchdir.ps1`)**: A dedicated script to safely force-unmount images, garbage collect, and clear stuck registry hives if a previous build fails or gets interrupted.
- **⚡ Custom WinPE Setup**: A streamlined, fully automated deployment process that handles disk partitioning and image application, completely bypassing the standard Windows setup screens.

### How to use the GUI:
1. Open **PowerShell** as Administrator.
2. Temporarily bypass the execution policy by running:
   ```powershell
